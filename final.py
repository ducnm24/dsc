...
# Reading in data from command line and generate test file
sys.argv = sys.argv[1:]
temp = list(sys.argv[0].split(','))
test = pd.DataFrame(columns = column_name, index = range(1))
for i in range(test.shape[1]):
    test.iloc[0, i] = np.float(temp[i])
...
train['return_1'] = train['return_2'] - train['lag_return_1']
test['return_1'] = test['return_2'] - test['lag_return_1']
...
# Data is fed line by line and prediction are written out right after

# In[ ]:


# Output
print(prediction(model, test_X))
